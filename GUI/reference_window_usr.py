...
class ReferencesWindowUser:
    ...
    def show_analysis_dialog(self):
        ...
        tk.Button(button_frame, text="Show Table", command=lambda: self.show_analysis_table(filter_var.get())).pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame, text="Show Graph", command=lambda: self.show_analysis_graph(filter_var.get())).pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame, text="Close", command=analysis_window.destroy).pack(side=tk.LEFT, padx=5)

    def show_analysis_table(self, filter_by):
    ...
    def show_analysis_graph(self, filter_by):
